...
public:
    ...
    // Connection funcs
        int getConnectionID(Neuron input, Neuron target) {
            ...
            for (int connectID : getOutputConnectionsID(input)) {
                ...
                if ((input.globalID == connect.inputItemIDs) && (target.globalID == connect.outputItemIDs)) { 
                    ID = connect.globalID;
                }
            }
            return ID;
    ...
    void generateDefaultNetwork() {
        SpikingNeuron sn; sn.globalID = allocateID(); sn.period = 4;
        Neuron n;         n.globalID    = allocateID(); n.threshold = 0.5f; n.weight = 1.0f;
        ActivationPoint ap; ap.globalID = allocateID(); ap.threshold = 0.5f;
        ...
        connectNodes(globalWeb, sn.globalID, NodeKind::SpikingNeuron, n.globalID, NodeKind::Neuron, 1.0f);
        connectNodes(globalWeb, n.globalID,  NodeKind::Neuron, ap.globalID, NodeKind::ActivationPoint, 1.0f);

        cout << "Generated default network: SpikingNeuron " << sn.globalID
            << " -> Neuron " << n.globalID << " -> ActivationPoint " << ap.globalID << endl;
    }
    ...
    void removeConnection(int connectionID) {
        ...
        conns.erase(std::remove_if(conns.begin(), conns.end(),
            [connectionID](const Connection& x) { return x.globalID == connectionID; }),
            conns.end());
    }
        ...
        switch(connectTo) {
            case 'a' : {
                for (ActivationPoint ap : web.ActivationPointInitalized) {
                    k.push_back(ap.globalID);
                }
                break;
            }
            case 'n' : {
                for (Neuron n : web.NeuronInitalized) {
                    k.push_back(n.globalID);
                }
                break;
            }
            default : {
    ...
    bool connectNodes(Web& web, int sourceID, NodeKind sourceKind, int targetID, NodeKind targetKind,
        ...
        switch (sourceKind) {
            case NodeKind::SpikingNeuron: {
                ...
                if (!src) return false;
                src->outputItemIDs.push_back(c.globalID);
                break;
            }
        ...
        switch (targetKind) {
            case NodeKind::Neuron: {
                ...
                if (!tgt) return false;
                tgt->inputItemIDs.push_back(c.globalID);
                break;
            }
    ...
    bool webGrader(Web& web) {
        ...
        for (Connection& c : web.ConnectionsInitalized) {
            ...
            if (c.activityScore < 0.1f) {
                weakConnections.push_back(c.globalID);
            }
        }
    ...
    void update(int currentTick, vector<float>* exportTo = nullptr) {
        ...
        for (Neuron& n : globalWeb.NeuronInitalized) {
            auto it = incoming.find(n.globalID);
        ...
        for (size_t i = 0; i < globalWeb.ActivationPointInitalized.size(); i++) {
            ...
            auto it = incoming.find(ap.globalID);
            float input = (it != incoming.end()) ? it->second.total : 0.0f;
    ...
    void runOscillationTest() {
        ...
        SpikingNeuron snA; snA.globalID = allocateID(); snA.period = 4;
        SpikingNeuron snB; snB.globalID = allocateID(); snB.period = 4;
        snB.ticksSinceLastFire = 2; // starts half a period ahead of A, so they alternate

        Neuron nA; nA.globalID = allocateID(); nA.threshold = 0.5f; nA.weight = 1.0f;
        Neuron nB; nB.globalID = allocateID(); nB.threshold = 0.5f; nB.weight = 1.0f;

        ActivationPoint apA; apA.globalID = allocateID(); apA.threshold = 0.5f;
        ActivationPoint apB; apB.globalID = allocateID(); apB.threshold = 0.5f;
        ...
        connectNodes(globalWeb, snA.globalID, NodeKind::SpikingNeuron, nA.globalID,  NodeKind::Neuron, 1.0f);
        connectNodes(globalWeb, snB.globalID, NodeKind::SpikingNeuron, nB.globalID,  NodeKind::Neuron, 1.0f);
        connectNodes(globalWeb, nA.globalID,  NodeKind::Neuron,        apA.globalID, NodeKind::ActivationPoint, 1.0f);
        connectNodes(globalWeb, nB.globalID,  NodeKind::Neuron,        apB.globalID, NodeKind::ActivationPoint, 1.0f);
