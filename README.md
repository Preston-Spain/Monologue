...
* Each has a diffrent grading protocal:
    ...
    * **`Spiking Neuron`** : Similar to `Neuron`, if a connection is not passing the threshold, the connection may be cut, similarly if a connection is consistanly activating the `weight` of the connection may go up. If the `Spiking Neuron` is consistanly not getting a lot of action, the `threshold` may begin to lower.
    * **`Activation point`** : If a connection is not passing it's `threshold` consistantly the connection may be cut, similarly if a connection is consistanly activating the `weight` of the connection may go up. If the `Activation point` is consistanly not getting a lot of action, the `threshold` may begin to lower.
...
* `Connection` limit : instead of a percent limit, each `Input`/`Output` is given a `weight` allowance and can allocate that weight between connections.
* Internal `weight`s for `Neuron`s : A `weight` to multiply the activation function by before emmiting it.
* `Neuron`s may need to grow to the next most connected `Neuron` in the series, so if `Neuron` *A* is connected to `Neuron` *B*, and `Neuron` *B* is connected to `Neuron`s *C*, *D*, *E*, with `weight`s of `.3`, `.5`, `.7` respectively, it may be benefical to have *A* connect to *E*. 
